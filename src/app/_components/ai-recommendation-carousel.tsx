...
import { getAllUmkmsForAI, getFavoriteUmkmsDetails, getUmkms } from '@/lib/actions'; // Kita pakai ulang `getUmkms`
    ...
    const fetchRecommendations = async () => {
      ...
      try {
        ...
        if (validUmkms.length === 0) {
          ...
          aiData.forEach(r => { reasonMap[r.slug] = r.reason; });
      ...
      <Carousel opts={{ align: 'start', loop: true }} className="w-full">
        <CarouselContent>
          {recommendations.map((umkm) => (
            <CarouselItem key={umkm.id} className="md:basis-1/2 lg:basis-1/3">
              <div className="p-1">
                <UmkmCard umkm={umkm} />
