                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {myUmkms.map((umkm) => {
                        ...
                        return (
                            <div
                                key={umkm.id}
                                onClick={isInteractionMode ? () => handleCardClick(umkm) : undefined}
                                className={`rounded-lg transition-all duration-200 ${
                                    isDeleteMode ? 'cursor-pointer ring-2 ring-destructive hover:ring-offset-2 hover:ring-offset-background' : ''
                                } ${
                                    isEditMode ? 'cursor-pointer ring-2 ring-primary hover:ring-offset-2 hover:ring-offset-background' : ''
                                }`}
                                role={isInteractionMode ? "button" : "article"}
                                tabIndex={isInteractionMode ? 0 : -1}
                                onKeyDown={isInteractionMode ? (e) => { if (e.key === 'Enter' || e.key === ' ') handleCardClick(umkm) } : undefined}
                            >
