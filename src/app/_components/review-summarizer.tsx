...
export default function ReviewSummarizer({ umkmName, reviews }: Props) {
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      setSummary('Maaf, terjadi kesalahan saat membuat ringkasan. Silakan coba lagi.');
          ...
          {isLoading ? (
            ...
            <p className="text-lg italic text-center">
              ...
              {summarySource === 'fallback' && (
                <span className="block text-sm mt-2 text-muted-foreground">(Ringkasan dibuat menggunakan fallback lokal)</span>
              )}
              {errorDetail && (
                <span className="block text-sm mt-2 text-red-500">Detail: {errorDetail}</span>
              )}
            </p>
          )}
        </div>
